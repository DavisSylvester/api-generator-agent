...
export function parseArgs(argv: readonly string[]): CliOptions {
  ...
  if (args.length === 0) {
    return { command: `help`, noDiagrams: false, noDocs: false };
  }
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case `-l`:
      case `--list-runs`:
        return { command: `list-runs`, noDiagrams: false, noDocs: false };
      ...
      case `--status`:
        ...
        return { command: `status`, statusRunId: next, noDiagrams: false, noDocs: false };
  ...
  return { command, prd, resume, iterations, maxTasks, concurrency, noDiagrams, noDocs };
}
...
function parseLegacy(args: readonly string[]): CliOptions {
  ...
  return { command: `run`, prd, iterations, maxTasks, noDiagrams: false, noDocs: false };
}
